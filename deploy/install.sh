...
if run "$PY" -m playwright install --with-deps chromium; then
    ...
    # روی سرورِ ایران این معمولاً ۴۰۳ جغرافیاییِ CDN است، نه خرابیِ واقعی.
    # `--with-deps` کتابخانه‌های سیستمی را با apt می‌گیرد (که کار می‌کند)؛ فقط
    # خودِ باینریِ مرورگر دانلود نمی‌شود.
    warn "دانلودِ کرومیوم شکست خورد (روی سرورِ ایران معمولاً ۴۰۳ جغرافیایی است)"
    warn "بعد از استارتِ ربات مالک این را بزن (از داخلِ تونل دانلود می‌کند):"
    warn "  sudo bash $APP_DIR/deploy/install-chromium.sh"
fi
