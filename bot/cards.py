...
#: نگاشتِ حروف/ارقامِ هم‌شکل به یک شکلِ واحد، برای **مقایسه‌ی متنی که کاربر تایپ
#: می‌کند**. صرفاً نمایشی نیست؛ یک باگِ واقعی را می‌بندد:
#:
#: کیبوردهای اندروید و iOS بعضاً «ی» عربی (U+064A) و «ک» عربی (U+0643) می‌فرستند،
#: در حالی که رشته‌ی داخلِ کد فارسی (U+06CC / U+06A9) است. پس `"ریست" == txt`
#: برای کاربری که ظاهراً درست تایپ کرده **False** می‌شود. روی سرور همین اتفاق
#: افتاد: تأییدِ «ریست» قبول نمی‌شد. ZWNJ و ارقامِ فارسی/عربی هم همین مشکل را
#: دارند.
_FOLD = str.maketrans({
    # ی و ک و ه و همزه‌ها
    "ي": "ی", "ى": "ی", "ئ": "ی", "ﯼ": "ی", "ﻱ": "ی",
    ...
    "ؤ": "و",
    # نشانه‌های نامرئی: ZWNJ/ZWJ/LRM/RLM/BOM
    "\u200c": "", "\u200d": "", "\u200e": "", "\u200f": "", "\ufeff": "",
    # اعرابِ اختیاری
    "\u064b": "", "\u064c": "", "\u064d": "", "\u064e": "", "\u064f": "",
    "\u0650": "", "\u0651": "", "\u0652": "", "\u0640": "",
...
def fold(text: str) -> str:
    """متنِ کاربر را برای **مقایسه** یک‌شکل می‌کند (نه برای نمایش).

    همیشه دو طرفِ مقایسه را fold کن، نه فقط ورودیِ کاربر را.
    """
    return (text or "").translate(_FOLD).strip()


def matches(text: str, *accepted: str) -> bool:
    """آیا متنِ کاربر با یکی از گزینه‌های پذیرفته می‌خواند (بدونِ حساسیت به شکل)؟"""
    got = fold(text).lower()
    return any(got == fold(a).lower() for a in accepted)
