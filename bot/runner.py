...
class JobManager:
    ...
    def __init__(self) -> None:
        ...
        # One trace code per login ATTEMPT, keyed by account. Jobs get their
        # trace from job_id, but the login path used to pass the literal string
        # "login" for every attempt -- so twenty failed logins produced twenty
        # cards all reading Trace: login and none of them could be found again.
        # This dict outlives _logins so the post-login steps (contact boost,
        # peer harvest) can still stamp the SAME code and stay correlated.
        self._login_trace: dict[str, str] = {}
    ...
    async def _boost_after_login(self, account: str, phone: str, head_text: str,
        ...
        try:
            ...
            await report(cards.error_card(
                ...
                detail=str(exc), phase="after_login",
                trace_id=self._login_trace.get(account, "")))
    ...
    async def start_bridge_login(self, account: str, phone: str, report: Report,
        ...
        if account in self._busy:
            return False
        self._busy.add(account)
        self._login_trace[account] = uuid.uuid4().hex[:6].upper()
        asyncio.create_task(self._bridge_login_job(account, phone, report, live,
                                                   card_factory=card_factory))
        return True

    def login_trace(self, account: str) -> str:
        """The trace code of this account's current/last login attempt."""
        return self._login_trace.get(account, "")
    ...
    async def _bridge_login_job(self, account: str, phone: str, report: Report,
        ...
        try:
            ...
            async with session_pool.lease(account, headed=config.HEADED_JOBS) as session:
                ...
                try:
                    # ONE number for every login path (config.LOGIN_TTL, default
                    # 320s). This used to be a hardcoded 300 here while the
                    # portal used 350 and .env.example said 600. The await sits
                    # inside the pool lease, so when it fires the browser is
                    # released -- an abandoned login cannot pin a Chromium.
                    code = await asyncio.wait_for(state.code_future,
                                                  timeout=config.LOGIN_TTL)
                except asyncio.TimeoutError:
                    await report(cards.card(
                        "⌛ LOGIN TIMEOUT",
                        [("Account", account)],
